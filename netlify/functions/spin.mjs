import { loadConfig, saveConfig, addAudit, json } from "./_lib.mjs";
...
export default async (req) => {
  ...
  if (!ticket) {
    await addAudit({ action: "spin_invalid", code });
    return json({ error: config.messages?.invalidCode || "Kode tiket tidak valid." }, 404);
  }

  if (ticket.status !== "active") {
    await addAudit({ action: "spin_expired", code });
  ...
  if (!prize) {
    prize = pickWeighted(config.wheel.segments);
  }
  if (!prize) {
    return json({ error: config.messages?.systemError || "Tidak ada hadiah aktif." }, 500);
  }
  ...
  await addAudit({ action: "spin", code, prizeId: prize.id });
